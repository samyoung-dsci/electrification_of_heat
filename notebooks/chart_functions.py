...
def chart_homes_waterfall(
    ...
    homes_output_spfh4 = homes_output[homes_output["SPF_type"] == "SPFH4"]["SPF_value"]
    exclusion_5 = (homes_output_spfh4 >= SPF_value_min_threshold) & (
        homes_output_spfh4 <= SPF_value_max_threshold
    )
    ...
    homes_output["Quality score outside threshold"] = homes_output["window_max_score"] >= 4
    ...
    # We also add 1 more at the moment as an interim adjustment so the total is as expected (was originally add 2 more)

    # 1st bar on waterfall
    total_installed = homes_output["Property_ID"].unique().size + 3

    # 2nd bar on waterfall

    homes_output = homes_output[homes_output["<1yr of data"] == False]
    lt_1yr_monitoring = homes_output["Property_ID"].unique().size

    # 3rd bar on waterfall
    homes_output = homes_output[homes_output["<1yr of usable data (early monitoring issues)"] == False]
    lt_1yr_usable = homes_output["Property_ID"].unique().size

    # 4th bar on waterfall
    homes_output = homes_output[homes_output["No valid 1yr window due to gaps"] == False]
    no_valid_1yr_window = homes_output["Property_ID"].unique().size

    # 5th bar on waterfall
    homes_output = homes_output[homes_output["SPF < 1.25 or SPF > 5.5"] == False]
    ...
    homes_output = homes_output[homes_output["Quality score outside threshold"] == False]
    ...
    remaining_homes = int(homes_waterfall.sum(axis=1)[0])
...
def SPF_h4_threshold_scatter(
    ...
    fig_data = data[(data["SPF_type"] == "SPFH4")]
    ...
    fig_scatter = px.scatter(
        fig_data,
        title="Heat Pump Energy Output vs SPFH4",
