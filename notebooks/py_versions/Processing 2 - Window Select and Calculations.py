...
for home in tqdm(files_list):
    ...
    if file_format == "csv":
        data = pd.read_csv(os.path.join(location_out_cleaned, "cleaned", f"{home}.csv")).set_index("Timestamp")
    elif file_format == "parquet":
        data = pd.read_parquet(os.path.join(location_out_cleaned, "cleaned", f"{home}.parquet")).set_index("Timestamp")

    home_summary_part = home_summary_cleaned[home_summary_cleaned["Property_ID"] == home]
    ...
    if "window_start" in home_summary_part.columns:
        home_summary_part = td.add_flow_temp_stats_for_window(
            data, home_summary_part, home, file_path=file_path, plot_path=plot_path
        )

    # Find spfs for coldest day
    home_summary_part_cold_day = td.add_spfs_for_coldest_period(data, home, pd.Timedelta(days=1), "Coldest_day_")

    # Find spfs for coldest half-hour
    home_summary_part_cold_HH = td.add_spfs_for_coldest_period(data, home, pd.Timedelta(minutes=30), "Coldest_HH_")
    ...
    if len(single_home_alteration_record) > 0:
        single_home_alteration_record["Property_ID"] = home
...
cleaning_flags["Property_ID"] = cleaning_flags["Property_ID"].str.split("=", expand=True)[1]
